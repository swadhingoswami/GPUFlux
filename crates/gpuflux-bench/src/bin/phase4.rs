...
fn main() -> Result<()> {
    ...
    let spec = ObjectSpec::new(17, size, DataLoc::GpuMemory);
    ...
    let engine_policy: Box<dyn Policy> = match policy.as_str() {
        ...
        "expected_cost" | "historical_cost" => Box::new(ExpectedCost),
    ...
    let cpu_util = samples
        ...
        .sum::<f64>()
        / samples.len() as f64;
    let nvme_lat = samples
        .iter()
        .map(|s| s.nvme_latency_us.unwrap_or(0.0))
        .sum::<f64>()
        / samples.len() as f64;

    println!("=== GPUFlux Phase 4: contention ===");
    ...
    println!(
        "contention    : cpu={} cores io={} readers",
        cpu_burn, io_readers
    );
    println!(
        "deadline      : {:?}",
    ...
    println!(
        "observed      : cpu_util={:.2} nvme_latency={:.0}us",
        cpu_util, nvme_lat
    );
